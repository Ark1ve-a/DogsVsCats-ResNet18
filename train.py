...
def train():
    ...
    for epoch in range(1, opt.nepoch + 1):
        ...
        for img, label in processBar:  # 循环读取封装后的数据集，其实就是调用了数据集中的__getitem__()方法，只是返回数据格式进行了一次封装
            ...
            accuracy = (predict == label).sum() / label.shape[0] # 计算正确率？

            processBar.set_description("[%d/%d] Loss: %.4f, Acc: %.4f" % (epoch, opt.nepoch, loss.item(), accuracy.item()))
